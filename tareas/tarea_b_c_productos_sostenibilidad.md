# Tarea 2 · Relación de THD con productos/servicios, sostenibilidad e investigación

## 🏢 Empresa seleccionada
Breve descripción de la empresa real o ficticia.

---

## 🧩 Relación entre THD y productos/servicios
Explica cómo la empresa podría usar cada tecnología para crear o mejorar un servicio digital.
Debes basarte en una aplicación real encontrada en noticias o informes.

| Tecnología | Producto/servicio digital | Caso real encontrado (enlace) | Beneficio para el cliente | Beneficio para la empresa |
|-----------|----------------------------|-------------------------------|----------------------------|---------------------------|
| IA | Sistema de monitorización ecológica del agua | Global Omnium + Telefónica Tech + Google Cloud analizan vídeos submarinos con IA para vigilar praderas de posidonia. https://www.telefonica.com/es/sala-comunicacion/prensa/global-omnium-telefonica-tech-se-unen-google-cloud-revolucionar-sector-agua-ia/?utm_source=chatgpt.com | Permite detectar amenazas ambientales (p. ej. deterioro de la posidonia) y reaccionar rápido, lo que mejora la preservación del entorno y el servicio medioambiental | Mejora la sostenibilidad, reduce riesgos, mejora la reputación, optimiza el mantenimiento y la prevención |
| IoT | Sensores para aparcamiento inteligente | Urbiotica usa sensores IoT (magnéticos, cámaras, NB‑IoT) para gestionar plazas de parking. https://urbiotica.com/us/success-stories/?utm_source=chatgpt.com | Los ciudadanos pueden encontrar plazas libres en tiempo real, menos tiempo buscando aparcamiento, menos emisiones | Reducción de congestión, datos sobre uso del aparcamiento, ahorro operativo, modelo de negocio de “ciudad inteligente” |
| Big Data | Plataforma de análisis de movilidad urbana | Urbiotica combina datos de sensores con analítica para optimizar movilidad. https://smartcitycluster.org/project/urbiotica/?utm_source=chatgpt.com | Mejora la experiencia del usuario (mejor guiado, predicción de ocupación) | Genera conocimiento sobre patrones de uso urbano, permite diseñar precios dinámicos, mejora la planificación urbana |
| Cloud Computing | Infraestructura para gestión de datos del ciclo hídrico | Global Omnium migra su gestión de datos a Google Cloud con Telefónica Tech. https://www.iagua.es/noticias/global-omnium/global-omnium-y-telefonica-tech-se-unen-google-cloud-dar-salto-tecnologico?utm_source=chatgpt.com | Servicio más fiable, escalable, con capacidad para análisis avanzado sin interrupciones | Ahorro en infraestructura, escalabilidad, reducción de costes, mayor agilidad para innovar |
| Blockchain | (Hipotético) plataforma de identidad digital para residentes | Muchas empresas planean usar blockchain para identidad digital. Ej: Repsol está desarrollando una cartera digital basada en blockchain para credenciales. https://forbes.es/listas/229627/lista-forbes-una-empresa-espanola-entre-las-50-que-mejor-usan-blockchain-en-el-mundo/?utm_source=chatgpt.com | Los ciudadanos pueden tener una identidad auto‑soberana, más control sobre sus datos | Mejora de confianza, nueva línea de negocio, transparencia, menor fraude, cumplimiento regulatorio |
| Ciberseguridad | Plataforma segura para todos los datos de sensores y usuarios | Según Kaspersky, muchas empresas ya usan IA e IoT y deben reforzar la ciberseguridad. https://www.kaspersky.es/about/press-releases/casi-la-mitad-de-las-empresas-utilizan-ia-e-iot-en-sus-negocios?utm_source=chatgpt.com | Los usuarios tienen garantía de que sus datos personales y los de sensores están seguros | Evita brechas, protege la reputación, asegura cumplimiento legal, reduce riesgos operativos |
| RA/RV | Aplicación de realidad aumentada para ciudadanos | (Hipotético, pero con base) Urbiotica menciona guiado mediante apps y paneles digitales; podría integrarse RA para mostrar plazas en tiempo real. También, según “Quality 4.0” la RA/RV se usan para mejorar calidad y operativa. https://arxiv.org/abs/1810.07829?utm_source=chatgpt.com | Ciudadanos verían a través de su móvil, con RA, dónde están plazas libres o rutas de parking, mejor interacción | Refuerza la innovación, fideliza usuarios, añade valor al servicio, permite campañas de concienciación (ej. movilidad sostenible) |

---

## 🌱 Impacto en la economía sostenible y eficiente
Describe cómo cada tecnología contribuye a la sostenibilidad y la eficiencia.

| Tecnología | Ahorro energético | Reducción de residuos | Mejora operativa | Riesgos/limitaciones |
|-----------|-------------------|-----------------------|------------------|----------------------|
| IA | Permite optimizar procesos (p. ej., predicción de fugas de agua) para consumir menos | Menor desgaste de infraestructuras gracias a mantenimiento predictivo | Procesos más eficaces, decisiones informadas, capacidad predictiva | Sesgo de algoritmos, coste de implementación, necesidad de datos de calidad |
| IoT | Sensores de bajo consumo (NB‑IoT, LoRaWAN) permiten monitorear sin gastar mucha energía | Evita sobreproducción o infrautilización (ej. plazas de aparcamiento vacías) | Datos en tiempo real, automatización, optimización del uso de recursos | Seguridad, gestión del gran volumen de datos, mantenimiento de sensores |
| Big Data | Analizar grandes volúmenes permite identificar y eliminar ineficiencias energéticas | Identificar patrones para evitar desperdicios de capacidad | Mejora la planificación, permite precios dinámicos y medidas basadas en datos | Privacidad, coste del almacenamiento, complejidad en el análisis |
| Cloud | Uso eficiente de recursos informáticos (escalado, optimización) | Menos hardware físico in situ => menos residuos electrónicos | Flexibilidad, agilidad, recuperación ante desastres, reducción de costes operativos | Dependencia del proveedor, consumo energético del centro de datos |
| Blockchain | Potencial para optimizar procesos (contratos, pagos) sin intermediarios físicos | Transacciones digitales que sustituyen procesos en papel | Transparencia, trazabilidad, confianza | Consumo energético (según tipo de blockchain), complejidad técnica, escalabilidad |
| Ciberseguridad | Al asegurar sistemas, se evita reinicios, pérdidas de datos y sobreequipamiento | Previene brechas que pueden suponer pérdidas masivas y residuos de sistemas | Protege la reputación, asegura continuidad, cumple regulaciones | Coste elevado, sofisticación de amenazas, necesidad de actualización constante |
| RA/RV | Puede reducir desplazamientos físicos si se utiliza para simulaciones o visitas virtuales | Menos necesidad de material físico para formación o demostraciones | Mejora la experiencia y formación, permite simulaciones sin impacto físico | Requiere dispositivo, consumo de potencia en móviles/ordenadores, barrera de adopción |

---

## 🔍 Investigación de casos reales (obligatoria)
Resume brevemente los casos y explica cómo podrían aplicarse a tu empresa.

- Caso IA: Global Omnium + Telefónica Tech + Google Cloud han implementado IA para analizar vídeos submarinos y monitorizar praderas de posidonia. 
Telefónica
Aplicación en nuestra empresa: Podríamos usar IA para monitorizar zonas naturales urbanas (ríos, parques) y alertar sobre degradación o amenazas ecológicas.

- Caso IoT: Urbiotica tiene sensores NB‑IoT y magnéticos para plazas de aparcamiento, que le permiten saber en tiempo real si hay coche o no. 
Urbiotica
Aplicación: Instalar sensores en la ciudad para gestionar el aparcamiento, reducir tráfico y emisiones, y ofrecer a los ciudadanos una App de “plaza inteligente”.

-Caso Big Data: En Urbiotica, usan la analítica de datos para gestionar la movilidad urbana y tomar decisiones sobre cómo se usan las plazas de aparcamiento. 
SmartCityCluster
Aplicación: Recoger datos históricos de uso de aparcamiento + datos en tiempo real para predecir demanda, ajustar precios o planificar nuevas zonas.

Caso Cloud: En la alianza Global Omnium / Telefónica / Google Cloud, migran su gestión hídrica a la nube, ganando escalabilidad y capacidad de análisis. 
iAgua

Aplicación: Nuestra plataforma de datos urbanos podría correr sobre cloud para procesar toda la información de sensores con flexibilidad, mantener la infraestructura ligera y escalar según demanda.

-Caso Blockchain: Si bien no hay un caso exacto de “ciudad inteligente + blockchain + parking/agua” citado, podemos inspirarnos en que Repsol desarrolla una “wallet” de identidad digital con blockchain. 
Forbes España
Aplicación: Crear una identidad digital para ciudadanos (“ID ciudadana”) basada en blockchain para acceder a servicios de SmartCity (parking, agua, transporte), con control de privacidad.

-Caso Ciberseguridad: Según Kaspersky, muchas empresas han adoptado IA e IoT, pero también destacan la necesidad de asegurar esos entornos. 
Kaspersky España
Aplicación: Implementar desde el inicio un sistema de seguridad robusto para proteger todos los sensores, datos en la nube y aplicaciones ciudadanas.

-Caso RA / RV: En el artículo “Quality 4.0” se menciona cómo RA/RV se usan para mejorar la calidad de procesos industriales y operativos. 
arXiv
Aplicación: Podríamos desarrollar una app de RA para que los ciudadanos vean en su móvil dónde están plazas libres, rutas sostenibles, o incluso simulaciones de cómo funcionará una nueva zona verde.

---

## 🧠 Reflexión final
- ¿Qué tecnología aporta más valor a los productos/servicios?
  Para SmartCity Sostenible, IA e IoT combinadas son las más valiosas: IoT para captar datos reales y en tiempo real, IA para analizarlos,   predecir y actuar.
  
- ¿Cuál genera mayor impacto sostenible?
  Probablemente IoT + Big Data + IA juntas, porque permiten optimizar recursos (agua, movilidad), reducir emisiones y evitar desperdicios.
- ¿Qué combinación sería más eficiente para la empresa?
  Una combinación de IoT + Cloud + Big Data + IA, con una capa de ciberseguridad para proteger todo, sería muy potente. Si además añadimos   RA como interfaz para los ciudadanos, se mejora mucho la experiencia.

## 📚 Fuentes consultadas
(Enlaces a las noticias o informes usados)
https://www.telefonica.com/es/sala-comunicacion/prensa/global-omnium-telefonica-tech-se-unen-google-cloud-revolucionar-sector-agua-ia/?utm_source=chatgpt.com
https://urbiotica.com/us/success-stories/?utm_source=chatgpt.com
https://smartcitycluster.org/project/urbiotica/?utm_source=chatgpt.com
https://www.iagua.es/noticias/global-omnium/global-omnium-y-telefonica-tech-se-unen-google-cloud-dar-salto-tecnologico?utm_source=chatgpt.com
https://forbes.es/listas/229627/lista-forbes-una-empresa-espanola-entre-las-50-que-mejor-usan-blockchain-en-el-mundo/?utm_source=chatgpt.com
https://www.kaspersky.es/about/press-releases/casi-la-mitad-de-las-empresas-utilizan-ia-e-iot-en-sus-negocios?utm_source=chatgpt.com
https://arxiv.org/abs/1810.07829?utm_source=chatgpt.com





